...
\end{figure}
The implementation is based on a Half Adder Counter on 32 bits, that allows to count from 0 to $2^{32} - 1$.
By mean of this, knowing the working frequency, the user can count the number of ticks and use the value to estimate the duration of a specific program in the following way:
\[
	\text{\textit{Execution time}} = \frac{1}{f_{ck}} \cdot CC
\]
Where $CC$ is computed as the difference between the starting and the end value of the Tick Timer .