...
The most important operation is the Instruction Decode. Here, after the identification of the instruction type, its field are extracted according to the general instruction structure, as described in Figure \ref{fig:rtype}, Figure \ref{fig:itype} and Figure \ref{fig:jtype}.\\

The Decode is based on the clever shown in Table \ref{table:decode_instr}. The table describes only the signals that goes towards the DataPath and not the internal ones.
