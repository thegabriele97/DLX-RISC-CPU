...
\begin{table}[H]
    ...
    \begin{tabular}{l|l}
        \textbf{Signal Name} & \textbf{Description}\\
        \hline
        PIPLIN\_IF\_EN & IF Stage enable: enable the of IR register \\
        IF\_STALL & IF Stage stall: a NOP is insterted in the IR register \\
        PC\_EN & Enable of the PC register \\
        JUMP\_EN & A JUMP must occur, used by the Decode Stage to compute the new PC \\
        CALL & The Register File must start the context switch (creation of a new window)\\
        ...
        UNSIGNED\_ID & Indicates that arithmetic units must consider operands as unsigned\\
