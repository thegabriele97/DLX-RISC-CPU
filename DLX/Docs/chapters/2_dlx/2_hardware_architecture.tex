...
R-Type Instructions are called in this way because all the operands are between registers. They all have in common the \emph{OP CODE} equal to \texttt{0b00000} and each instruction is differentiated from another one thanks to the \emph{FUNC CODE} field, as shown in Table \ref{table:r_type}.
...
\begin{tabularx}{\textwidth}{|l|c|l|l|X|}
    ...
    \hline
    SLL & \texttt{0x04} & SLL R3, R2, R1 & $R3 \leftarrow R2 \ll R1$ & Logical shift left\\ 
    \hline
    SRL & \texttt{0x06} & SRL R3, R2, R1 & $R3 \leftarrow R2 \gg R1$ & Logical shift right\\ 
    \hline
    ROR & \texttt{0x08} & ROR R3, R2, R1 & $R3 \leftarrow R2 \circlearrowright R1$ & Right rotation\\ 
    \hline
    ROL & \texttt{0x09} & ROL R3, R2, R1 & $R3 \leftarrow R2 \circlearrowleft R1$ & Left rotation\\ 
    \hline
    MUL & \texttt{0x0E} & MUL R3, R2, R1 & $R3 \leftarrow R2 * R1$ & Integer multiplication\\ 
    \hline
    ADD & \texttt{0x20} & ADD R3, R2, R1 & $R3 \leftarrow R2 + R1$ & Integer Add\\ 
    \hline
    ADDU & \texttt{0x21} & ADDU R3, R2, R1 & $R3 \leftarrow R2 + R1$ & Integer Add\\ 
    \hline
    SUB & \texttt{0x22} & SUB R3, R2, R1 & $R3 \leftarrow R2 - R1$ & Integer subtraction\\ 
    \hline
    SUBU & \texttt{0x23} & SUBU R3, R2, R1 & $R3 \leftarrow R2 - R1$ & Integer subtraction\\ 
    \hline
    AND & \texttt{0x24} & AND R3, R2, R1 & $R3 \leftarrow R2 \texttt{\&} R1$ & Bitwise AND\\ 
    \hline
    OR & \texttt{0x25} & OR R3, R2, R1 & $R3 \leftarrow R2 \mid R1$ & Bitwise OR\\ 
    \hline
    XOR & \texttt{0x26} & XOR R3, R2, R1 & $R3 \leftarrow R2 \oplus R1$ & Bitwise XOR\\ 
    \hline
\end{tabularx}
...
\begin{tabularx}{\textwidth}{|l|c|l|l|X|}
    ...
    MNEMONIC & FUNC CODE & EXAMPLE & OPERATION & DESCRIPTION \\ 
    \hline
    SEQ & \texttt{0x28} & SEQ R3, R2, R1 & $R3 \leftarrow R2 == R1$ & 1 if R2 eqaul R1\\ 
    \hline
\end{tabularx}
